...
def run_story_benchmark(args):
    ...
    if valid_mask.sum() > 0:
        labels, confidences = metrics.classify_with_confidence(
            coverage=results_df.loc[valid_mask, "coverage_expr"].values,
    ...
    fig_a = plotting.plot_archetype_scatter(
        coverage=results_df.loc[eval_mask, "coverage_expr"].values,
    ...
    for arch in ["housekeeping", "regional_program", "sparse_noise", "niche_marker"]:
        ...
        if mask.sum() > 0:
            ax_a.scatter(
                results_df.loc[eval_mask, "coverage_expr"].values[mask],
    ...
    im = ax_b.imshow(cm_norm, cmap="Blues", aspect="auto", vmin=0, vmax=1)
    ...
    try:
        coverage_fg = results_df.loc[eval_mask, "coverage_expr"].values
...
"""
    ...
    prec_20_val = prec_20[0] if len(prec_20) > 0 else np.nan
