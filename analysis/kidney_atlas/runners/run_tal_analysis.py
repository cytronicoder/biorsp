...
def plot_gene_panel(
    ...
    if plot_mode == "absolute":
        ...
        plot_radar(
            radar, ax=ax_radar2, title="Distal ($R < 0$)", mode="distal", theta_convention="math"
        )
    else:
        plot_radar(
            radar, ax=ax_radar, title=f"{symbol} RSP", mode="signed", theta_convention="math"
        )
    ...
    info_lines = [
        ...
        f"Sign: {sign_str}",
        f"$\\bar{{R}}$: {_fmt(r_mean)}",
        f"Coverage$_{{geom}}$: {_fmt(cov_geom)}",
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("BioRSP Case Study: TAL Cells")
    logger.info("=" * 60)
