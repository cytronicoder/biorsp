...
def test_selection_bias_wedge_vs_global_rim():
    ...
    # Use same vantage for both (origin)
    v = np.array([0.0, 0.0])

    # Compute polar coords and normalize
    ...
    # Config with empty_fg_policy='zero' and delta=60 (not 180)
    # With delta=60, wedge pattern won't fill all sectors
    ...
    # Key assertions
    # 1. Wedge should have lower coverage_fg (fewer sectors have sufficient foreground)
    # This is the critical test - wedge is localized, so not all sectors have FG
    assert (
    ...
    # 2. Both should be negative (rim pattern)
    assert (
    ...
    # 3. Verify empty_fg_policy is working (no all-NaN in RSP profiles)
    # Under policy='zero', empty FG sectors should get R=0, not NaN
...
def test_selection_bias_parameter_sweep():
    ...
    # For each delta, verify wedge < global in |R_mean|
...
def test_empty_fg_policy_comparison():
    ...
    # For wedge rim, this difference should be noticeable
    assert (
