...
def test_strata_merging():
    ...
    # Case 2: One bin too small, should merge
    indices = get_strata_indices(r, theta, n_r_bins=3, min_stratum_size=40, mode="radial")
    # 100 / 3 = 33.3 cells per bin.
    # Bin 0: 33, Bin 1: 33, Bin 2: 34
    # Bin 0 (33) < 40 -> merge with Bin 1 (33) -> 66.
    # Bin 2 (34) < 40 -> merge with previous -> 100.
...
def test_p_value_finite_correction():
    ...
    # If observed is always >= null (e.g. mode="none" and same seed might lead to same stat)
    # p = (count_ge + 1) / (9 + 1)
...
def test_mask_freezing():
    ...
    theta = np.random.uniform(0, 2 * np.pi, n)
    y = np.zeros(n)
    y[:20] = 1.0  # 10% foreground

    config = BioRSPConfig(B=10, min_fg_sector=2)
    adequacy = assess_adequacy(r, theta, y, config=config)
    valid_mask = adequacy.sector_mask

    # Run RSP with frozen mask
    # Even if we permute y such that some sectors have 0 fg,
    # the RSP should still be computed for those sectors (returning 0.0)
    y_perm = np.random.permutation(y)
    radar_perm = compute_rsp_radar(r, theta, y_perm, config=config, frozen_mask=valid_mask)

    # The resulting RSP should have non-NaN values exactly where valid_mask is True
    # (unless the sector was empty in the original geometry, but here we use same r, theta)
