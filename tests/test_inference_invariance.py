...
def test_geometry_mask_fixed_across_permutations():
    ...
    # Run multiple times with different FG patterns
    masks = []
    weights = []
    ...
    # Geometry masks should be identical (or very similar)
    # since spatial structure is the same
    if len(masks) >= 2:
        for i in range(1, len(masks)):
            # Masks should be identical since they depend on total count + robust scale
            # which are determined by spatial distribution, not FG labels
            ...
            # Weights depend on total count, also invariant
            np.testing.assert_array_equal(
                weights[0],
                weights[i],
...
def test_permutation_uses_observed_mask():
    ...
    # The radar result should have coverage metrics
    if radar.geom_supported_mask is not None:
...
def test_sector_weights_invariant_to_fg_pattern():
    ...
    # Different FG patterns
    y1 = (rng.random(n_cells) > 0.3).astype(float)
    y2 = (rng.random(n_cells) > 0.7).astype(float)  # Much fewer FG

    radar1 = compute_rsp_radar(r, theta, y1, config=config)
    radar2 = compute_rsp_radar(r, theta, y2, config=config)

    # Weights should be identical (depend on total count, not FG/BG split)
    np.testing.assert_array_equal(
...
def test_no_selection_bias_in_coverage():
    ...
    # Sparse expression (few FG)
    y_sparse = (rng.random(n_cells) > 0.9).astype(float)
    # Dense expression (many FG)
    ...
    # Geometry coverage should be identical
