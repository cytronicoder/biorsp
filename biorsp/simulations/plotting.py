...
def plot_qq(
    ...
    perm_floor: Optional[float] = None,
) -> Figure:
    """
    ...
    figsize : tuple, optional
        Figure size
    perm_floor : float, optional
        Permutation test floor = 1/(K+1). If provided, draws horizontal line
        to help interpret plateaus (not miscalibration, just resolution limit).
    ...
    if perm_floor is not None and perm_floor < alpha:
        ax.axhline(
            ...
            alpha=0.7,
            linewidth=1.5,
            label=f"Perm floor={perm_floor:.4f}",
        )
        ax.text(alpha * 0.95, perm_floor + 0.002, "1/(K+1)", fontsize=8, ha="right", color="orange")
...
def plot_marker_recovery(
    ...
    ax.bar(range(len(k_vals)), prec_vals, color="#FF5722", alpha=0.8, edgecolor="black")

    for i, (_k, p) in enumerate(zip(k_vals, prec_vals)):
...
def compose_story_onepager(
    ...
    fig.add_gridspec(2, 2, hspace=0.3, wspace=0.25)

    def copy_panel(source_fig, target_ax, label):
        source_ax = source_fig.axes[0]

        for _collection in source_ax.collections:
            pass
...
def plot_support_diagnostics(
    ...
    coverage_geom : np.ndarray
        Geometry coverage (fraction of sectors with sufficient cells)
    spatial_score : np.ndarray
        Spatial organization scores (S)
    true_archetypes : np.ndarray
        Ground truth archetype labels
    ...
    ax = axes[2]
    ratio = coverage_fg / (coverage_geom + 1e-9)
    for archetype in unique_archetypes:
        mask = true_archetypes == archetype
        color = ARCHETYPE_COLORS.get(archetype, "#888888")
        ax.scatter(
...
def plot_precision_with_baseline(
    ...
    for i, (_k, f) in enumerate(zip(k_vals, fold_vals)):
...
def plot_genegene_example_panel(
    ...
    boundaries = []
    for _i, m in enumerate(unique_modules[:-1]):
