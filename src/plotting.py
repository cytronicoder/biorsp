...
def plot_rsp_heatmap(
    ...
    # Plot heatmap
    im = ax.pcolormesh(theta_grid, r_grid, result.Z_heat,
    ...
    if show_peak and result.phi_star is not None:
        ...
        ax.plot([peak_angle, peak_angle], [0, J], 'g-', 
               linewidth=2, alpha=0.7, label='Peak direction')
...
def plot_rsp_grid(
    ...
    for i, result in enumerate(results):
        plot_rsp_heatmap(result, ax=axes[i], cmap=cmap, 
                        show_peak=show_peaks, show_colorbar=True)
...
def plot_rsp_summary(
    ...
    ax2.axvline(np.mean(feature_values), color='red', linestyle='--', 
...
def save_top_results(
    ...
    for i, result in enumerate(results):
        ...
        safe_name = str(result.name or f'feature_{i}').replace('/', '_').replace('=', '_')
